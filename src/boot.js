...
window.__loadError = (error) => {
  ...
  const graphics=/WebGPU|WebCuda|graphics|device|adapter/i.test(String(error?.message||error));
  ...
  document.getElementById('load-caption').textContent = graphics ? 'WebCuda requires WebGPU on localhost or HTTPS and hardware acceleration.' : 'Check your connection, then reload the coastline.';
};

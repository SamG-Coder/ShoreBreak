...
const source=(await readFile(new URL('../ShoreBreak.cu',import.meta.url),'utf8')).split('// ORIGINAL GRAPHICS STAGES')[0];
await mkdir(new URL('../.qa/faithful/',import.meta.url),{recursive:true});
const entries=[...source.split('// ORIGINAL GRAPHICS STAGES')[0].matchAll(/__global__\s+void\s+(\w+)\s*\(/g)].map(m=>m[1]);
